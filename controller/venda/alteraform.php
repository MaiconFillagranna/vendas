...
$DADOS->financeiro = [];
buscaClassificacaoFinanceiraParcela($Dados);
while ($reg = pg_fetch_object($retorno)) {
    ...
    if(isset($reg->vfcentrocusto)){
        $cccod =$reg->vfcentrocusto;
    ...
    $reg->vfdatavencimento = inverte_data($reg->vfdatavencimento, '-');
    $reg->vfvalor = number_format($reg->vfvalor, 2, ',', '.');
    $reg->vfcusto = number_format($reg->vfcusto, 2, ',', '.');
    if (!empty($reg->vfclassifin)) {
...
$topnome = '';
$gerafin = '';
verificaTipoOperacao($DADOS);
if ($DADOS->vnftipooperacao) {
    $topID = $DADOS->vnftipooperacao;
    $sql6 = "SELECT tonome, togerafinanceiro, tooperacao,toespcodigo FROM tipooperacao WHERE tocodigo = $DADOS->vnftipooperacao AND nccodigo = $empresa";
    ...
    while ($tipooperacao = pg_fetch_object($retorno)) {
        ...
        $tooperacao = $tipooperacao->tooperacao;
...
if ($DADOS->vcontabancariacod) {
    $sql = "SELECT cbnomeoriginal FROM contabancaria WHERE nccodigo =" . $empresa . " AND cbcodigo = " . $DADOS->vcontabancariacod;
    $retorno = pg_query($DB_CONN, $sql);
    while ($reg = pg_fetch_assoc($retorno)) {
        $fincontbancnome = $reg["cbnomeoriginal"];
    }
} else {
    $sql = "SELECT cbnomeoriginal,cbcodigo FROM contabancaria WHERE nccodigo =" . $empresa . " AND cbpadrao = 1";
    $retorno = pg_query($DB_CONN, $sql);
    while ($reg = pg_fetch_assoc($retorno)) {
        $fincontbancnome = $reg["cbnomeoriginal"];
        $DADOS->vcontabancariacod = $reg["cbcodigo"];
    }
//    $fincontbancnome = '';
}
...
function verificaTipoOperacao($Dados) {
    if($Dados->vnftipooperacao === null) {
        $SQL = "SELECT CASE WHEN clitipooperacao is not null
                            THEN clitipooperacao
                            WHEN topadrao = 1
                            THEN tocodigo
                            ELSE NULL
                        END as tipooperacao
                  FROM tipooperacao
             LEFT JOIN cliente
                    ON cliente.nccodigo = tipooperacao.nccodigo
                 WHERE cliente.nccodigo = $1
                   AND cliente.clicodigo = $2
                   AND topadrao = $3";
        $oQuery = new Query();
        $oQuery->setSQL($SQL);
        $oQuery->setParams([$Dados->nccodigo, $Dados->vcodcli, 1]);
        $oLinha = $oQuery->getFirstRow();
        $Dados->vnftipooperacao = $oLinha->tipooperacao;
    }
}

function buscaClassificacaoFinanceiraParcela($Dados) {
    foreach($Dados->financeiro as $financeiro) {
        if($financeiro->vfclassifin === null && isset($Dados->vnftipooperacao)) {
            $SQL = "SELECT toclassificacaofinanceira, cfnome
                      FROM tipooperacao
                INNER JOIN classificacaofinanceira
                        ON classificacaofinanceira.nccodigo = tipooperacao.nccodigo
                       AND tipooperacao.toclassificacaofinanceira = classificacaofinanceira.cfcodigo
                     WHERE tipooperacao.nccodigo = $1
                       AND tipooperacao.tocodigo = $2";
            $oQuery = new Query();
            $oQuery->setSQL($SQL);
            $oQuery->setParams([$Dados->nccodigo, $Dados->vnftipooperacao]);
            $financeiro->vfclassifin = $oQuery->getFirstRow()->toclassificacaofinanceira;
            $financeiro->cfnome = $oQuery->getFirstRow()->cfnome;
        }
    }
}
        ...
        <form id="formVenda" method="post" autocomplete="off">
            <div class="row">
                <div class="col-md-5" id="divFornecedor">
                    ...
                    <label>Cliente*</label>
                    <input id="pessoa" type="text" style="margin-bottom: 10px;" autocomplete="off" class="form-control " name="pessoa" data-required="required"value="<?php echo $nomepessoa; ?>"/>
                ...
                <div class="col-md-7" style="padding-right: 0px;padding-left:5px;">
                    ...
                    <div class="col-md-6" id="divTipoOperacao" style="padding-right: 5px;padding-left:0px;">
                        ...
                        <input id="tipooperacaoid" type="text" style="margin-bottom: 10px;display: none;" autocomplete="off" class="form-control " name="tipooperacaoid" data-required="required"value="<?php echo $topID ?>"/>
            ...
            <div class="col-md-3 alinhaMd3" id="divCB" style="margin-top: 30px">
                ...
                <input id="ContBanc" type="text" style="margin-bottom: 10px;" data-required="required" autocomplete="off" class="form-control" name="vcontabancarianome" value="<?php echo $fincontbancnome; ?>"/>
            ...
            <div id="BLOCO" style="display: none">
                <div class="row">
                    ...
                    <!--<label>Nossas ObservaÃ§Ãµes</label><textarea class="form-control" rows="3" id="vobsempresa" name="vobsempresa"></textarea>-->
                        <label>Nossas Observações</label><textarea <?php echo $pago; ?> class="form-control" rows="3" id="vobsempresa" name="vobsempresa" onkeyup="contarCaracteres($(this), 2000)"><?php echo $DADOS->vobsempresa; ?></textarea>
                    </div>
                </div>
            </div>
            <legend></legend>
...
//                    console.log($(this).serialize());
                    jQuery.ajax({
                        type: "POST",
                        url: "<?php echo $URL_BASE_PATH ?>/modulos/venda/altera.php",
                        data: $(this).serialize(),
                        success: function () {
                            swal({
                                title: "Sucesso!",
