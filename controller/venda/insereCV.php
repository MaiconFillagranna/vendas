...
$Cadastra->vfdatavencimento = CampoTexto::of(["nome" => "dataparcela", "obrigatorio" => true, "multiplo" => true]); // TODO fazer campo data
...
if ($Cadastra->optionsRadios->getValor() <> 'A'){
...
class CadastraVenda {
    ...
    public function inserir() {
        ...
        $sql = "INSERT INTO venda ("
                ...
                . "vatualizada, vnftipooperacao, vordemcompra, vvalorcusto, vclassifin, vvaloracrescimo, vnfoutrasdespesas) VALUES ("
                . "$1,$2,$3,$4,$5,"
        ...
        executaSqlTransacao($sql, [
            ...
            $this->vordemcompra->getValor(), $this->vvalorcusto->getValor(), $this->vclassifin->getValor(), 
            $this->vvaloracrescimo->getValor(), $this->vvaloracrescimo->getValor()
        ]);
        ...
        $sql = executaSql("SELECT nccomissoes FROM nossocliente WHERE nccodigo = $1", [$empresa]);
        $geracomissao = getLinhaQuery($sql)->nccomissoes;
        if ($geracomissao === 'S') {
            $this->insereComissao();
        }
    }
    ...
    public function inserirItens() {
        ...
        $sql = "INSERT INTO vendaitens (nccodigo, vcodigo, vicodigo, vparcial, vicodservproduto, vinome, vivalorunitario, viquantidade, vinfcfop, vinfpesoliquido, vinfpesobruto, vium, vincm, vivalorcustototal,vimovestoque)
...
//        if ($this->vvalordesconto->getValor()) { // FUNCAO RATEIO DE DESCONTO DE ITENS PARA A NOTA || LUIS
        ...
        $percentual_desconto = $desconto * 100 / $valormercadorias;
        ...
        while ($vendaitens = pg_fetch_object($retorno)) {
            ...
            $valorDescontoItem = $valorTotItem * $percentual_desconto / 100;
            $valorDescontoItem = intval($valorDescontoItem * 100) / 100;
            $desconto_aux = $desconto_aux + $valorDescontoItem;
            ...
            $sql = "UPDATE vendaitens SET vivalordesconto = $4, vivalorfrete = $5 WHERE nccodigo = $1 AND vcodigo = $2 AND vicodigo = $3";
            executaSqlTransacao($sql, [
                $empresa,
                $this->codigo,
                $vicodigo,
            ...
            if ($this->comissao == 'R' && $this->optionsRadios->getValor() == 'P') { // se recebe comissão por recebimento COMISSAO 02/05/2017
                ...
                $total_prod = $valorTotItem - $valorDescontoItem;
        ...
        if ($desconto_aux <> $desconto) { //pega o resto para somar na item com valor maior
            $resto_desconto = $desconto - $desconto_aux;
            $valorDescontoItem = $valorDescontoItem + $resto_desconto;
            $sql = "UPDATE vendaitens SET vivalordesconto = $4 WHERE nccodigo = $1 AND vcodigo = $2 AND vicodigo = $3";
            executaSqlTransacao($sql, [
                $empresa,
                $this->codigo,
                $vicodigo,
...
//        }if desconto
    ...
    public function inserirFinanceiro() {
        ...
        for ($i = 0; $i < $this->qtdparcela; $i++) {
            ...
            $sql3 = "INSERT INTO vendafinanceiro (nccodigo, vcodigo, vfparcela, vparcial, vfvalor, vfobs, vfdatavencimento, vfclassifin, vftipopagamento, vfcusto,vfespcodigo,vfcentrocusto)
